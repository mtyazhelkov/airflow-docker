...
def arrivals_flights():
     ...
     URL= f"https://opensky-network.org/api/flights/arrival?airport=EDDF&begin={fst_ts}&end={lst_ts}"
     query='''
             -- Используем json.dumps для превращения списка в одну JSON-строку
             -- jsonb_populate_recordset автоматически разложит ключи по колонкам таблицы flights.tb_arrivals
             insert into flights.tb_arrivals (
                  icao24,
                  firstseen,
